...
$nome = mysqli_real_escape_string($db, $_POST['nome'] . ' ' . $_POST['snome']) ;
...
if ($id_array == $id) {
  ...
  if ($insert1 && $insert2) {
   ...
   echo"<script language='javascript' type='text/javascript'>
   alert('Usuário cadastrado com sucesso!');window.location.href='indexCadastro.php'
   </script>";
  ...
  /** Exibe mensagem de erro caso não inserido **/
   ...
   echo"<script language='javascript' type='text/javascript'>
   alert('Não foi possível cadastrar esse usuário!');window.location.href='indexCadastro.php'
   </script>";
